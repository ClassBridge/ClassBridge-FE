...
export default function PaymentSuccessPage({ params }: Props) {
  ...
  const { data: classData } = useClassSummaryData(params.id);
          ...
          <div className="flex flex-col gap-3 w-[764px] py-6 px-10 rounded border border-primary">
            ...
            <div className="space-x-12">
              <span className="font-normal text-sm text-black">
                ...
                <span className="pr-2.5 border-r border-gray-light">
                  {formatDateToLocaleString(
                    new Date(reservationData.lesson.time),
                  )}
                </span>
                <span className="pl-2.5">{`${formatTimeToString(new Date(reservationData.lesson.time))} - ${formatTimeToString(new Date(new Date(reservationData.lesson.time).getMinutes() + classData.duration))}`}</span>
              </span>
              <span className="font-normal text-sm text-black">
                <span className="font-bold">{"결제 금액"}</span>
                {" : "}
                {`${(classData.price * parseInt(reservationData.quantity)).toLocaleString()} 원`}
              </span>
            </div>
          </div>
        )}
