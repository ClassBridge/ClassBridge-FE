...
import type { ReservationDetailResponseData } from "@/app/api/reservations/[reservationId]/type";
      ...
      <div className="flex gap-12">
        ...
        <div className="space-y-4 w-full py-4">
          ...
          <div className="space-y-2 font-normal text-sm text-black">
            <span className="block">{classData.tutorName}</span>
            <span className="block">{classData.address}</span>
            <div>
              <span className="pr-2.5 border-r border-gray-light">
                {formatDateToLocaleString(new Date(data.lesson.lessonDate))}
              </span>
              <span className="px-2.5 border-r border-gray-light">
                {`${data.lesson.startTime.slice(0, 5)} - ${data.lesson.endTime.slice(0, 5)}`}
              </span>
              <span className="pl-2.5">{`${data.quantity}인`}</span>
            </div>
          </div>
          <span className="block w-full text-end font-bold text-base text-black">{`${(classData.price * data.quantity).toLocaleString()}원`}</span>
        </div>
      </div>
    </div>
    // <div className="space-y-7 w-full px-32">
